...
def create_table_with_schema(client, table_id, schema_sample_data):
    """Create a BigQuery table with schema based on sample data with robust error handling"""
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "Not found" in str(check_error):
                try:
                    print(f"Creating table {table_id} (attempt {attempt + 1}/{max_retries})...")
                    sample_df = pd.DataFrame(schema_sample_data)
                    
                    # Configure job with better settings
                    ...
                    job.result(timeout=300)  # 5 minutes timeout
                    print(f"Successfully created table: {table_id}")
                    return True
                    
                except Exception as create_error:
                    if attempt < max_retries - 1:
                        print(f"Attempt {attempt + 1} failed: {create_error}")
                        ...
                        print(f"Failed to create table {table_id} after {max_retries} attempts: {create_error}")
                        return False
            else:
                print(f"Unexpected error checking table {table_id}: {check_error}")
                return False
...
def main():
    ...
    if not PROJECT_ID:
        print("ERROR: PROJECT_ID not set. Please set GCP_PROJECT_ID or BIGQUERY_PROJECT_ID environment variable.")
        return False
    
    if not DATASET:
        print("ERROR: DATASET not set. Please set BIGQUERY_DATASET environment variable.")
        return False
    ...
    try:
        ...
        print("Please check your credentials and project permissions.")
        return False
    
    # Validate schema data
    if not schemas:
        print("ERROR: No schema definitions found.")
        return False
    
    print(f"Found {len(schemas)} table schemas to create...")
    
    # Create all tables with progress tracking
    success_count = 0
    failed_tables = []
    total_count = len(schemas)
    
    for i, (table_id, sample_data) in enumerate(schemas.items(), 1):
        print(f"\n[{i}/{total_count}] Processing table: {table_id}")
        
        # Validate sample data
        if not sample_data:
            print(f"WARNING: No sample data for {table_id}, skipping...")
        ...
        try:
            if create_table_with_schema(client, table_id, sample_data):
                success_count += 1
                print(f"SUCCESS: {table_id}")
            else:
                failed_tables.append(table_id)
                print(f"FAILED: {table_id}")
        except Exception as e:
            print(f"ERROR: Unexpected error with {table_id}: {e}")
            failed_tables.append(table_id)
    ...
    if success_count == total_count:
        print("SUCCESS: All tables are ready! You can now run the data generator.")
        return True
    else:
        print(f"WARNING: {len(failed_tables)} tables failed to create:")
